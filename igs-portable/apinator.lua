...
function IGS.GetSign(tParams, secret)
	...
	return util.SHA256(s .. (secret or IGS.SECRET))
end
...
function IGS.RawQuery(sMethod, tParams, fOnSuccess, fOnError)
	if not IGS.SECRET then
		IGS.print(Color(255,0,0), error_no_igsmod)
		return
	end

	if #IGS.SECRET ~= 32 or not IGS.SECRET:match("^[a-f0-9]+$") or IGS.SECRET == 0 then
		IGS.print(Color(255,0,0), error_invalid_credentials)
		return
	end

	IGS.DoRequest(IGS.UID, IGS.SECRET, sMethod, tParams, fOnSuccess, fOnError)
end
...
function IGS.SetSharedKV(key, value, ttl, cb)
	kvapi.set(IGS.SECRET .. ":" .. key:URLEncode(), value, ttl, cb)
end
-- IGS.SetSharedKV("test4", "тест", 60, PRINT)

function IGS.GetSharedKV(key, cb)
	kvapi.get(IGS.SECRET .. ":" .. key:URLEncode(), cb)
end
